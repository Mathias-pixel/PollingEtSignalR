...
export class PollingComponent implements OnInit {
  ...
  async complete(id: number) {
    // TODO On invoke la méthode pour compléter une tâche sur le serveur (Contrôleur d'API)
      return await lastValueFrom(this.http.get<any>(this.url + "/UselessTask/Complete" + id)); 
  }

  async addtask() {
    // TODO On invoke la méthode pour ajouter une tâche sur le serveur (Contrôleur d'API)
   this.tasks.push(await lastValueFrom(this.http.post<UselessTask>(this.url + "/UselessTask/Add" + this.taskname, null))); 
  }
  ...
  async updateTasks() {
    ...
    this.tasks.push(await lastValueFrom(this.http.get<UselessTask>(this.url + "/UselessTask/Getall"))); 
